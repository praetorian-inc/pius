...
- `pkg/whoisfreaks` SSL certificate client — single-domain WhoisFreaks `ssl/live` lookups (leaf + optional chain, parsed fields + raw PEM), header-driven per-category rate limiting, and credit accounting (`+1 per 2 chain certs`, capped) (OFFSEC-2444)
