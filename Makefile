# Makefile for Pius - Organizational Asset Discovery Tool

.PHONY: all build test test/cover lint clean install help cli-docs
.DEFAULT_GOAL := help
.DELETE_ON_ERROR:
...
build: $(BUILD_DIR)/$(BINARY) ## Build pius binary

$(BUILD_DIR)/$(BINARY): $(GO_SOURCES) | $(BUILD_DIR)
	$(GO) build $(LDFLAGS) -o $@ ./cmd/pius
