...
func TestWhoisFreaks_Run_NoRecordAtAllIsAnError(t *testing.T) {
	p := newTestWhoisFreaks(t, func(w http.ResponseWriter, r *http.Request) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "no WHOIS record")
}

// Verbatim live response for siemens.de, 2026-07-29. WhoisFreaks returns NO raw
// WHOIS text for ccTLDs — no whois_raw_domain, no registry_data — so a raw-only
// reader emitted nothing for every ccTLD asset and discarded real contacts.
const liveWhoisFreaksCcTLD = `{
  "status": true,
  "domain_registered": "yes",
  "registrant_contact": {
    "name": "Siemens AG",
    ...
    "email_address": "domainmanagement.cc@siemens.com"
  }
}`

func TestWhoisFreaks_Run_FallsBackToStructuredContactForCcTLD(t *testing.T) {
	p := newTestWhoisFreaks(t, func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(liveWhoisFreaksCcTLD))
	})

	findings, err := p.Run(context.Background(), plugins.Input{Domain: "siemens.de"})
	require.NoError(t, err)

	got := map[string]string{}
	for _, f := range findings {
		got[f.Data["preseed_type"].(string)] = f.Value
		assert.Equal(t, "whoisfreaks", f.Source)
	}
	assert.Equal(t, map[string]string{
		"whois+name":  "Siemens AG",
		"whois+email": "domainmanagement.cc@siemens.com",
	}, got)
}

// bbc.co.uk's only contact field is Nominet's redaction placeholder. It must not
// become a whois+email pivot — it would match every redacted .uk domain. The
// record exists, so this is a successful empty result, not an error.
func TestWhoisFreaks_Run_RedactedStructuredContactYieldsNoPivots(t *testing.T) {
	p := newTestWhoisFreaks(t, func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"status":true,"domain_registered":"yes",` +
			`"registrant_contact":{"email_address":"redacted@nominet.uk"}}`))
	})

	findings, err := p.Run(context.Background(), plugins.Input{Domain: "bbc.co.uk"})
	require.NoError(t, err, "a fully redacted record is not an error")
	assert.Empty(t, findings)
}

// The raw path must keep precedence: gTLDs return both, and the raw record is
// richer than the vendor's parsed block.
func TestWhoisFreaks_Run_RawTakesPrecedenceOverStructured(t *testing.T) {
	p := newTestWhoisFreaks(t, func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"domain_registered":"yes",` +
			`"whois_raw_domain":"Domain Name: acme.com\nRegistrant Organization: From Raw\n",` +
			`"registrant_contact":{"company":"From Structured"}}`))
	})

	findings, err := p.Run(context.Background(), plugins.Input{Domain: "acme.com"})
	require.NoError(t, err)
	require.Len(t, findings, 1)
	assert.Equal(t, "From Raw", findings[0].Value)
}
