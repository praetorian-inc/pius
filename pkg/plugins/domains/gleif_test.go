...
func TestGLEIFPlugin_Run_ConfidenceSignals_Parent(t *testing.T) {
	...
	resolutionReferences := confidenceReferences(t, f.Confidences[0])
	require.Len(t, resolutionReferences, 2)
	assert.Contains(t, referenceURL(t, resolutionReferences[0]), srv.URL+"/fuzzycompletions?")
	assert.Equal(t, srv.URL+"/lei-records/LEI001", referenceURL(t, resolutionReferences[1]))
	assert.Equal(t, 50, f.Confidences[1].Score)
	assert.Contains(t, f.Confidences[1].Justification, "direct parent")
	relationshipReferences := confidenceReferences(t, f.Confidences[1])
	require.Len(t, relationshipReferences, 2)
	assert.Equal(t, srv.URL+"/lei-records/LEI_PARENT", referenceURL(t, relationshipReferences[0]))
	assert.Equal(t, srv.URL+"/lei-records/LEI001/direct-parent-relationship", referenceURL(t, relationshipReferences[1]))
	assert.Equal(t, plugins.ConfidenceHigh, plugins.TotalConfidence(f))
...
func TestGLEIFPlugin_Run_ConfidenceSignals_Sibling_DifferentJurisdiction(t *testing.T) {
	...
	references := confidenceReferences(t, sib.Confidences[1])
	require.Len(t, references, 3)
	assert.Equal(t, srv.URL+"/lei-records/LEI010", referenceURL(t, references[0]))
	assert.Equal(t, srv.URL+"/lei-records/LEI010/direct-parent-relationship", referenceURL(t, references[1]))
	assert.Equal(t, srv.URL+"/lei-records/LEI001/direct-parent-relationship", referenceURL(t, references[2]))
