...
func TestDoHEnumPlugin_Run_RateLimitRetryReportsSuccessfulEndpoint(t *testing.T) {
	...
	assert.Contains(t, finding.Confidences[0].References[0].URL, successfulEndpoint.URL)
	assert.Contains(t, finding.Confidences[0].References[0].URL, "name=www.example.com&type=A")
