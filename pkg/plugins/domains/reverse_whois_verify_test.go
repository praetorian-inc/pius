...
func TestViaWHOIS_ReferralHopFailureReportsIncomplete(t *testing.T) {
	...
	wantHops := append([]string{defaultServer, "whois.tld.example"},
		slices.Repeat([]string{"whois.registrar.example"}, whoisHopAttempts)...)
	assert.Equal(t, wantHops, hops,
...
func TestViaWHOIS_WhoisQueryErrorCarriesNoIncompletenessReason(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantHops:        slices.Repeat([]string{defaultServer}, whoisHopAttempts),
...
func TestViaWHOIS_UnparseableSalvagedRecordKeepsItsIncompletenessReason(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// An expired ctx short-circuits whoisHopWithRaw's retry after one dial; a
			// clean one lets it exhaust whoisHopAttempts on the failing registrar hop.
			registrarDials := whoisHopAttempts
			if tc.want == whoisIncompleteDeadline {
				registrarDials = 1
			}
			wantHops := append([]string{defaultServer, tldServer},
				slices.Repeat([]string{registrarServer}, registrarDials)...)
			assert.Equal(t, wantHops, hops,
