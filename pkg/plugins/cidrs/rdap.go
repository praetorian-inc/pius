...
func newRDAPPlugin(cfg rdapConfig) *RDAPPlugin {
	return newRDAPPluginWithDoer(cfg, client.New())
}

// newRDAPPluginWithDoer creates an RDAPPlugin that issues every request through
// the given doer. It is the seam the exported per-registry constructors are
// built on; callers are responsible for passing a usable doer (see
// doerOrDefault).
func newRDAPPluginWithDoer(cfg rdapConfig, doer httpDoer) *RDAPPlugin {
	return &RDAPPlugin{cfg: cfg, doer: doer}
}

// doerOrDefault adapts an injected client into an httpDoer.
//
// Nil rule, uniform across every exported constructor in this package: a nil
// *client.Client means "no preference" and yields client.New() — the same
// default newRDAPPlugin and the init() registrations have always used. The
// fallback is load-bearing, not cosmetic: assigning a nil *client.Client
// directly into an httpDoer field produces a non-nil interface holding a nil
// pointer, so the first request would panic dereferencing Client.http instead
// of returning an error.
func doerOrDefault(c *client.Client) httpDoer {
	if c == nil {
		return client.New()
	}
	return c
