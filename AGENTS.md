...
- Attach source material with `plugins.AddConfidenceWithReference(finding, score, justification, reference)`. Each confidence has at most one serialized `Reference`: a human label, a renderer `Type`, and arbitrary JSON-compatible `Data`. Multiple protocol records are bundled as a `references` reference without increasing the confidence score.
