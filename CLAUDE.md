...
- **`verifyCandidates`** compares the resolved registrant org against the query org (**Jaccard** over normalized token sets — `jaccardTokenSets`: shared tokens ÷ *union*, so both sides' distinguishing tokens count against the score) and assigns per-candidate confidence rather than one flat score:
  ...
  - mismatch — similarity < 0.30 **and** neither token set contains the other → `confReverseWhoisMismatch` = 0.40 (needs review)
  - **Sparse overlap is not the de-rank test; disagreement is (ENG-5172).** Because Jaccard divides by the union, a short query org fully contained in a longer registrant sinks on length asymmetry alone — `Walmart` vs `Walmart Global Enterprises Holdings` is `{walmart} ⊂ {walmart, global, enterprises, holdings}` = 1/4 = 0.25, under the floor — so the bare threshold ranked a plausible subsidiary *below* a candidate whose WHOIS never resolved at all. `tokenSetContained` gates the de-rank arm: containment in **either** direction (the query may be the longer name) is under-specification, not contradiction, so a contained pair falls through to `confReverseWhoisUnverified` (0.50). De-ranking to 0.40 now requires that each side contribute a token the other lacks.
  - **Calibration basis: 0.60/0.30 are confirmed, not tuned (ENG-5374).** The cutoffs are recorded as-is against an *enumeration* of the metric rather than a fixture corpus (`TestJaccardReachabilityByTokenCount`): with `kq`/`kc` the distinct token counts and `m` the shared count, similarity is always `m/(kq+kc-m)`, so which arm can fire is fully determined by the shape `(kq,kc,m)`. Three consequences are load-bearing. **(1)** At `kq=kc=1` the reachable set is exactly `{0.0, 1.0}`, so *every* cutoff in `(0,1]` classifies that shape identically — moving 0.60 to 0.7/0.8/0.95 changes no verdict, which is why neither ENG-5172 nor ENG-5374 is a re-tuning. **(2)** Jaccard *already* requires ≥ 2 shared distinct tokens to corroborate: `1/(kq+kc-1) ≥ 0.60` forces `kq+kc ≤ 2`, so `kq=kc=1` — both sides the same single token, i.e. the legitimate "Praetorian" vs "Praetorian Inc" — is the only single-shared-token shape that corroborates, and that bound is exact rather than a search limit. Under the old containment coefficient `m / min(kq,kc)` the same question admitted an *unbounded* family (every `(1,kc)` shape scored 1.0 on one shared token), which was the ENG-5374 defect. **An explicit shared-evidence floor on top of Jaccard is therefore redundant, and must not be added:** `"Acme Acme"` vs `"Acme"` has distinct sets that are the same singleton (Jaccard 1.0, correctly corroborated) but differ as joined strings, so such a floor would de-rank it. **(3)** What is *not* establishable hermetically is that 0.60/0.30 are precision-**optimal** — that is a claim about a decision boundary and needs the real ViewDNS/Whoxy return distribution plus per-candidate ownership labels of independent provenance. Hand-authored fixtures cannot substitute, because their labels would come from the same judgment that picked the threshold. That work requires production data and stays in **ENG-5166**; the metric's own multiset/set asymmetry in `tokenSimilarity` (still scored against by `github-org`'s pinned floor assertion) is **ENG-5890**.
