...
- `plugins.NeedsReview(f)` is derived, never stored: `len(f.Confidences) > 0 && TotalConfidence(f) < ConfidenceHigh`. An unscored finding (deterministic RDAP expansion, crt.sh) has made no ambiguity claim and returns `false`; an explicit zero-score entry has, and returns `true`. That `len() > 0` guard is the distinction the slice exists for — the emitters key their nil-vs-materialized behavior off it.
