...
- Attach source material with `plugins.AddConfidenceWithReference(finding, score, justification, reference)`. Each confidence has at most one `Reference`: a human label, a renderer `Type`, and arbitrary JSON-compatible `Data`. Bundle records from one API response into that reference; create another confidence only for an independently scored signal.
- Prefer raw source records or credential-free request/response exchanges over vague links. `plugins.HTTPReference` deliberately records method, URL, request body, and response without authentication headers. Never put API keys, authorization headers, cookies, or other credentials in reference data.
